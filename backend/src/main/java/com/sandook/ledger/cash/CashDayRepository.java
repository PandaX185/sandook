...
public interface CashDayRepository extends JpaRepository<CashDay, Long> {
    ...
    @Query(value = """
            SELECT cd.id            AS id,
                   ...
                   COALESCE(SUM(cd.sales_minor + cd.extra_minor - cd.withdraw_minor - cd.deposit_minor)
                       OVER (PARTITION BY cd.book_id ORDER BY cd.date), 0) AS balanceMinor
            FROM cash_days cd
    ...
    /** Opening balance for a day = cumulative net of all strictly-earlier days in the book. */
    @Query(value = """
